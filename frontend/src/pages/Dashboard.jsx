...
function StatCard({ label, value, sub, icon: Icon, delay = 0, testId }) {
  return (
    <div
      className={`bg-white rounded-xl border border-[#E7E5E4] p-6 animate-fade-up-delay-${delay}`}
      data-testid={testId}
    >
      ...
      <div className="font-heading text-3xl font-semibold text-[#1C1917] tracking-tight">{value}</div>
      {sub && <div className="text-sm text-[#78716C] mt-1">{sub}</div>}
    </div>
  );
}
    ...
    <Layout>
      <div className="p-8 md:p-12 max-w-[1400px]">
        <div className="flex items-start justify-between mb-10 animate-fade-up">
          <div>
            <div className="text-[11px] uppercase tracking-widest text-[#78716C] mb-2">Overview</div>
            <h1 className="font-heading text-4xl font-semibold text-[#1C1917]">Tableau de bord</h1>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
              <div className="lg:col-span-2 bg-white rounded-xl border border-[#E7E5E4] p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <div className="text-[11px] uppercase tracking-widest text-[#78716C]">
                      Évolution mensuelle
                    </div>
                    <h2 className="font-heading text-xl font-semibold text-[#1C1917] mt-1">
                ...
                <div className="h-64">
                  {kpis.monthly_trend?.length ? (
                    ...
                    <div className="h-full flex items-center justify-center text-[#78716C] text-sm">
              ...
              <div className="bg-white rounded-xl border border-[#E7E5E4] p-6">
                ...
                <h2 className="font-heading text-xl font-semibold text-[#1C1917] mb-5">
                  Avancement des étapes
                </h2>
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="text-[#57534E]">Validées</span>
                      <span className="font-medium text-[#047857]">{kpis.totals.validated_steps}</span>
                    </div>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-[#57534E]">En attente validation</span>
                    <span className="font-medium text-[#0369A1]">{kpis.totals.pending_validations}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[#57534E]">Total étapes</span>
                    <span className="font-medium text-[#1C1917]">{kpis.totals.total_steps}</span>
                  </div>
                  {kpis.totals.pending_validations > 0 && (
                    <button
                      onClick={() => navigate("/validations")}
                      data-testid="dashboard-validations-cta"
                      className="w-full mt-3 h-10 rounded-lg border border-[#E7E5E4] bg-[#E0F2FE] text-[#0369A1] text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-[#BAE6FD] transition"
                    >
            ...
            <div className="bg-white rounded-xl border border-[#E7E5E4] overflow-hidden">
              <div className="px-6 py-5 border-b border-[#E7E5E4] flex items-center justify-between">
                <div>
                  <div className="text-[11px] uppercase tracking-widest text-[#78716C]">Portefeuille</div>
                  <h2 className="font-heading text-xl font-semibold text-[#1C1917] mt-1">Vos sites</h2>
                </div>
                <button
                  onClick={() => navigate("/sites")}
                  className="text-sm text-[#B84B31] hover:text-[#993D26] font-medium flex items-center gap-1"
              ...
              {kpis.per_site.length === 0 ? (
                <div className="p-10 text-center">
                  <div className="text-[#78716C] mb-4">Aucun site créé pour l'instant.</div>
                  <button
                    onClick={() => navigate("/sites/new")}
                    data-testid="create-first-site-btn"
                    className="h-11 px-5 rounded-xl bg-[#B84B31] hover:bg-[#993D26] text-white font-medium transition inline-flex items-center gap-2"
                  >
                ...
                <table className="w-full">
                  <thead className="bg-[#FDFBF7]">
                    <tr>
                      <th className="text-left px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">Site</th>
                      <th className="text-left px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">Niche</th>
                      <th className="text-left px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">Avancement</th>
                      <th className="text-right px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">CA</th>
                      <th className="text-right px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">Marge</th>
                      <th className="text-right px-6 py-3 text-[11px] uppercase tracking-widest text-[#78716C] font-medium">ROAS</th>
                    </tr>
                  </thead>
                  <tbody>
                    {kpis.per_site.map((s) => (
                      <tr
                        ...
                        className="border-t border-[#E7E5E4] hover:bg-[#FDFBF7] cursor-pointer transition"
                        data-testid={`site-row-${s.id}`}
                      >
                        <td className="px-6 py-4">
                          <div className="font-medium text-[#1C1917]">{s.name}</div>
                          <div className="text-xs text-[#78716C] mt-0.5">
                            {s.current_step_title ? `Étape ${s.current_step_number} · ${s.current_step_title.substring(0, 40)}` : "Non démarré"}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-[#57534E]">{s.niche}</td>
                        <td className="px-6 py-4 w-56">
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-2 bg-[#F5F2EB] rounded-full overflow-hidden">
                              <div
                                className="h-full bg-[#B84B31] rounded-full transition-all"
                                style={{ width: `${s.progress_pct}%` }}
                              />
                            </div>
                            <span className="text-xs font-medium text-[#57534E] min-w-[40px] text-right">
                              {s.progress_pct}%
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right font-medium text-[#1C1917]">{fmt(s.revenue)}</td>
                        <td className="px-6 py-4 text-right font-medium text-[#047857]">{fmt(s.margin)}</td>
                        <td className="px-6 py-4 text-right font-medium text-[#1C1917]">
