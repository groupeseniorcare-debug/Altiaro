    ...
    <Layout>
      <div className="p-8 md:p-12 max-w-[1400px]">
        <div className="mb-10 animate-fade-up">
          <div className="text-[11px] uppercase tracking-widest text-zinc-500 mb-2">Activité récente</div>
          <h1 className="text-3xl font-semibold text-zinc-100">Activité des concepteurs</h1>
        ...
        {loading ? (
          ...
          <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-zinc-100 mb-2">Aucune activité pour l'instant</h3>
          ...
          <div className="space-y-3">
            {items.map((step, i) => (
              <button
                ...
                className={`w-full flex items-center gap-5 text-left bg-zinc-950 rounded-xl border border-zinc-800 p-5 hover:border-[#B84B31]/50 hover:shadow-sm transition-all duration-200 animate-fade-up-delay-${Math.min(i + 1, 4)}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm text-zinc-500 font-medium">{step.site?.name}</span>
                    <span className="text-xs text-zinc-500">· Phase {step.phase} · Étape #{step.number}</span>
                  </div>
                  <div className="font-heading font-semibold text-zinc-100">{step.title}</div>
                  <div className="text-sm text-zinc-400 mt-1 line-clamp-1">{step.summary}</div>
                </div>
