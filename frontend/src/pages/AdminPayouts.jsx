    ...
    <Layout>
      <div className="p-6 md:p-10 max-w-[1500px]">
        ...
        <div className="mb-8 animate-fade-up">
          <div className="text-[11px] uppercase tracking-widest text-neutral-500 mb-2">Administration · Trésorerie</div>
          <h1 className="text-3xl font-semibold text-neutral-900">Virements Concepteurs</h1>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <button
            onClick={load}
            className="h-10 px-4 rounded-xl border border-neutral-200 bg-white hover:border-[#B84B31] text-sm flex items-center gap-2 transition"
          ...
          <button
            ...
            className="h-10 px-5 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-white text-sm font-medium flex items-center gap-2 transition disabled:opacity-40"
...
function KpiCard({ icon: Icon, color, label, value, hint, testId }) {
  return (
    <div
      className="bg-white rounded-md border border-neutral-200 p-5 hover:shadow-md transition"
      data-testid={testId}
    >
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 py-3 text-sm font-medium border-b-2 transition -mb-px ${
        active ? "border-[#B84B31] text-neutral-900" : "border-transparent text-neutral-500 hover:text-neutral-900"
      }`}
    >
      {children}
    </button>
...
function EmptyState({ title, subtitle }) {
  return (
    <div className="bg-white rounded-md border border-dashed border-neutral-200 p-16 text-center">
      <Bank size={40} weight="thin" className="mx-auto text-neutral-400 mb-3" />
      <div className="text-base font-medium text-neutral-900">{title}</div>
      <div className="text-sm text-neutral-500 mt-1.5">{subtitle}</div>
    </div>
  );
}
...
function PendingTable({ items, onMarkPaid, onCancel, copyIban, copiedIban }) {
  ...
  return (
    <div className="bg-white rounded-md border border-neutral-200 overflow-hidden" data-testid="pending-table">
      <table className="w-full">
        ...
        <tbody>
          {items.map((p) => (
            <tr key={p.id} className="border-t border-neutral-200 hover:bg-white transition" data-testid={`pending-row-${p.id}`}>
              <td className="px-5 py-4">
                <div className="font-medium text-neutral-900">{p.concepteur_name || p.holder_name || "—"}</div>
                <div className="text-xs text-neutral-500">{p.concepteur_email || ""}</div>
              </td>
              <td className="px-5 py-4">
                <div className="font-mono text-sm text-neutral-900">{maskIban(p.iban)}</div>
                <div className="text-xs text-neutral-500">{p.bic ? `BIC ${p.bic}` : ""} · {p.holder_name}</div>
              </td>
              <td className="px-5 py-4 text-right">
                <div className="text-base font-semibold text-neutral-900 tabular-nums">{fmt(p.amount)}</div>
              </td>
              <td className="px-5 py-4 text-right text-sm text-neutral-500 tabular-nums">{fmtDate(p.created_at)}</td>
              <td className="px-5 py-4">
                <div className="flex items-center gap-1.5 justify-end">
                  <button
                    onClick={() => copyIban(p.iban, p.id)}
                    data-testid={`copy-iban-${p.id}`}
                    className="h-9 px-3 rounded-lg border border-neutral-200 hover:border-[#B84B31] text-xs flex items-center gap-1.5 transition"
                  >
                    {copiedIban === p.id ? (
                  ...
                  <button
                    ...
                    className="h-9 px-3 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-medium flex items-center gap-1.5 transition"
                  >
  ...
  return (
    <div className="space-y-3" data-testid="preview-list">
      {rows.map((r) => {
        ...
        return (
          <div key={r.user_id} className="bg-white rounded-md border border-neutral-200 overflow-hidden">
            <div className="p-5 flex items-center gap-4 flex-wrap md:flex-nowrap">
              <div className="flex-1 min-w-0">
                <div className="text-base font-semibold text-neutral-900">{r.name || "Concepteur"}</div>
              ...
              <div className="text-right">
                <div className="text-[11px] uppercase tracking-widest text-neutral-500">À virer</div>
                <div className="text-xl font-bold text-neutral-900 tabular-nums" data-testid={`net-due-${r.user_id}`}>
                  {fmt(r.net_due_eur)}
                </div>
              </div>
              <button
                ...
                className="h-10 w-10 rounded-lg border border-neutral-200 hover:border-[#B84B31] text-neutral-500 flex items-center justify-center transition"
              >
            ...
            {open && (
              <div className="border-t border-neutral-200 bg-white p-5 space-y-4">
                {/* IBAN block */}
                {r.has_iban ? (
                  <div className="bg-white rounded-xl border border-neutral-200 p-4 flex items-center gap-3 flex-wrap">
                    <Bank size={18} className="text-neutral-500" />
                    <div className="flex-1 min-w-[200px]">
                      <div className="text-[11px] uppercase tracking-widest text-neutral-500">IBAN</div>
                      <div className="font-mono text-sm text-neutral-900 break-all">{formatIban(r.iban)}</div>
                    ...
                    <button
                      ...
                      className="h-10 px-4 rounded-xl border border-neutral-200 hover:border-[#B84B31] text-sm flex items-center gap-2 transition"
                    >
                ...
                {r.site_breakdown && r.site_breakdown.length > 0 && (
                  <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
                    <div className="px-4 py-3 border-b border-neutral-200 text-[11px] uppercase tracking-widest text-neutral-500">
                      Ventilation par site
                    </div>
                    <table className="w-full text-sm">
                      <thead className="bg-white text-[11px] uppercase tracking-wider text-neutral-500">
                        <tr>
                      ...
                      <tbody>
                        {r.site_breakdown.map((s) => (
                          <tr key={s.site_id} className="border-t border-neutral-200">
                            <td className="px-4 py-2">
                              <Link to={`/sites/${s.site_id}`} className="text-neutral-900 hover:text-neutral-900 transition">
                                {s.site_name}
                              </Link>
                            </td>
                            ...
                            <td className="px-4 py-2 text-right tabular-nums font-semibold text-neutral-900">{fmt(s.concepteur_share)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
function HistoryTable({ items }) {
  ...
  return (
    <div className="bg-white rounded-md border border-neutral-200 overflow-hidden" data-testid="history-table">
      <table className="w-full">
        <thead className="bg-white text-[11px] uppercase tracking-widest text-neutral-500">
          <tr>
            <th className="text-left px-5 py-3">Concepteur</th>
        ...
        <tbody>
          {items.map((p) => (
            <tr key={p.id} className="border-t border-neutral-200 hover:bg-white transition">
              <td className="px-5 py-4">
                <div className="font-medium text-neutral-900">{p.concepteur_name || p.holder_name}</div>
                <div className="text-xs text-neutral-500">{p.concepteur_email || ""}</div>
              </td>
              <td className="px-5 py-4 font-mono text-sm text-neutral-500">{maskIban(p.iban)}</td>
              <td className="px-5 py-4 text-right font-heading tabular-nums text-emerald-400">{fmt(p.amount)}</td>
              <td className="px-5 py-4 text-right text-sm text-neutral-500 tabular-nums">{fmtDate(p.paid_at)}</td>
              <td className="px-5 py-4 text-right text-xs text-neutral-500">{p.paid_by || "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function MiniStat({ label, value, color }) {
  return (
    <div className="text-right px-3 border-r border-neutral-200 last:border-0">
