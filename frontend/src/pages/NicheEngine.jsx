      ...
      <div className="p-6 md:p-12 max-w-5xl mx-auto">
          ...
          <div className="mt-8">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-16 pl-14 pr-48 rounded-md border-2 border-zinc-800 bg-zinc-950 focus:ring-4 focus:ring-[#B84B31]/15 focus:border-[#B84B31] outline-none text-[17px] transition shadow-sm"
              />
              <button
                onClick={() => launch()}
            ...
            <div className="flex flex-wrap gap-2 mt-3" data-testid="suggestion-chips">
              <span className="text-xs text-zinc-500 py-1.5 pr-1">Idées :</span>
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => { setProduct(s); }}
                  className="px-3 py-1.5 rounded-full text-xs bg-zinc-950 border border-zinc-800 hover:border-[#B84B31] hover:text-zinc-100 transition"
                >
                  {s}
                </button>
              ))}
            ...
            {analyzing && (
              <div className="mt-4 p-4 rounded-xl bg-[#FDF4E7] border border-[#F5E0C3] flex items-start gap-3" data-testid="analyzing-bar">
                <Sparkle size={18} weight="fill" className="text-zinc-100 mt-0.5 animate-pulse" />
                <div className="flex-1">
                  <div className="font-medium text-zinc-100 text-sm">L'IA analyse 6 marchés en profondeur...</div>
        ...
        {history.length > 0 && (
          <section className="mb-12">
            <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 mb-3">Tes analyses récentes</h2>
              ...
              {history.map((h) => {
                ...
                return (
                  <button
                    ...
                    className="text-left bg-zinc-950 rounded-xl border border-zinc-800 p-4 hover:border-[#B84B31] transition group"
                  >
                    ...
                    <div className="font-medium text-zinc-100 text-sm group-hover:text-zinc-100 transition line-clamp-2">
        ...
        {catalog.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-xs text-zinc-500">Basé sur 20 produits qualifiés</div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3" data-testid="catalog-grid">
              {catalog.map((n) => (
                <button
                  ...
                  className="text-left bg-zinc-950 rounded-xl border border-zinc-800 p-3.5 hover:border-[#B84B31] hover:shadow-sm transition group"
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="text-xl">{n.emoji}</div>
                    {n.hero && <Target size={12} weight="fill" className="text-zinc-100" />}
                  </div>
                  <div className="text-xs font-medium text-zinc-100 line-clamp-2 group-hover:text-zinc-100 transition">
                    {n.name}
                  </div>
