...
function StepRow({ s, onToggleValidate, validating }) {
  ...
  const pill = (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-neutral-500 mt-0.5 truncate">{s.subtitle}</div>
      </div>
  ...
  return (
    <div className="flex items-stretch gap-2" data-testid={`journey-row-${s.key}`}>
      <div className="flex-1 min-w-0">
        <Tag {...props} className="block">{pill}</Tag>
      </div>
      ...
      <button
        ...
        title={s.done ? "Annuler la validation" : "Marquer cette étape comme validée"}
        className={`px-3 rounded-xl border text-xs font-medium transition whitespace-nowrap flex items-center gap-1.5 disabled:opacity-50 ${
          s.done
      ...
      >
        {validating ? "…" : s.done ? (<><CheckCircle size={12} weight="fill" /> Validée</>) : "Valider"}
      </button>
    </div>
  );
}
