...
function StepRow({ step, meta, href }) {
  const { Icon } = meta;
  // Source de vérité = backend `/journey` payload :
  //   - `is_clickable` (booléen explicite, prioritaire)
  //   - sinon fallback sur `blocked_by_previous && !completed`
  ...
  const softUnlocked = step.soft_unlocked === true && !done && !locked;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          ...
          {softUnlocked && (
            <span
              className="text-[9px] uppercase tracking-widest bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded whitespace-nowrap font-semibold"
              data-testid={`journey-soft-${step.key}`}
            >
              En cours
            </span>
          )}
        ...
        <div className="text-xs text-neutral-500 mt-0.5 truncate" title={step.reason}>
          {locked
            ...
            : (softUnlocked ? (step.soft_reason || step.reason) : step.reason)}
        </div>
      </div>
        ...
        {!locked && (
          <>
            <span className={`text-[11px] font-medium ${
              done ? "text-emerald-700" : softUnlocked ? "text-amber-700" : "text-neutral-600"
            }`}>
              {done ? "Terminé" : "Ouvrir"}
            </span>
