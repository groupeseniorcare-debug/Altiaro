    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[55] pointer-events-none">
          <div className="absolute inset-0 bg-neutral-900/30 pointer-events-auto" onClick={() => setOpen(false)} />
          <div
          ...
          >
            ...
            {/* Sessions dropdown */}
            {showSessions && (
              <div className="border-b border-neutral-200 bg-neutral-100/40 p-3 max-h-56 overflow-y-auto">
                {sessions.length === 0 ? (
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {messages.length === 0 && !busy && (
                <div className="flex flex-col items-center text-center py-6">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#B84B31] to-[#D97706] flex items-center justify-center mb-3">
                    <Robot size={28} weight="duotone" className="text-neutral-900" />
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5 w-full">
                    {SUGGESTIONS.map((s, i) => (
                      <button
                        ...
                        className="text-left text-sm p-2.5 rounded-lg border border-neutral-200 hover:border-[#B84B31] hover:bg-neutral-100/40 transition"
                      >
                        {s}
                      </button>
                    ))}
              ...
              {busy && (
                <div className="flex items-center gap-2 text-sm text-neutral-500" data-testid="copilot-thinking">
                  <div className="w-7 h-7 rounded-full bg-neutral-200 flex items-center justify-center">
                    <Sparkle size={12} weight="fill" className="text-neutral-900 animate-pulse" />
                  </div>
            ...
            <div className="border-t border-neutral-200 p-3">
              <div className="flex items-end gap-2 bg-neutral-100/40 rounded-xl p-2 focus-within:ring-2 focus-within:ring-[#B84B31]/30 transition">
                <textarea
                  rows={1}
                  value={input}
                  onChange={(e) => {
  ...
  if (role === "user") {
    return (
      <div className="flex justify-end" data-testid="copilot-msg-user">
        <div className="max-w-[85%] bg-neutral-900 text-white rounded-md rounded-tr-sm px-3.5 py-2.5 text-sm">
          {msg.content}
        </div>
      </div>
    );
    ...
    return (
      <div className="flex gap-2" data-testid="copilot-msg-assistant">
        <div className="shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-[#B84B31] to-[#D97706] flex items-center justify-center mt-1">
          <Sparkle size={12} weight="fill" className="text-neutral-900" />
        </div>
        <div className="flex-1 min-w-0">
          {msg.tool_trace && msg.tool_trace.length > 0 && (
            <details className="mb-2 text-xs">
              <summary className="cursor-pointer text-neutral-500 hover:text-neutral-900 flex items-center gap-1">
                <Wrench size={11} />
                {msg.tool_trace.length} outil{msg.tool_trace.length > 1 ? "s" : ""} utilisé{msg.tool_trace.length > 1 ? "s" : ""}
              </summary>
              <div className="mt-1.5 space-y-1">
                {msg.tool_trace.map((t, i) => (
                  <div key={i} className="bg-neutral-100/40 rounded p-2 font-mono text-[10px]">
                    <div className="font-semibold text-neutral-900">
                      {t.name}({Object.keys(t.arguments || {}).join(", ")})
                    </div>
                    {t.thought && <div className="text-neutral-500 italic mt-0.5">{t.thought}</div>}
                  </div>
                ))}
              </div>
            </details>
          )}
          <div className="bg-neutral-100/40 rounded-md rounded-tl-sm px-3.5 py-2.5 text-sm text-neutral-900 whitespace-pre-wrap">
            {msg.content}
          </div>
        </div>
      </div>
